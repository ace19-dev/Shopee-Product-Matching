...
def main():
    ...
    trainset = CassavaDataset(data_dir=args.dataset_root,
                              fold=['train_fold0_21642.npy'],
                              csv=['train.csv'],
                              mode='train',
    ...
    valset = CassavaDataset(data_dir=args.dataset_root,
                            fold=['valid_fold0_5411.npy'],
                            csv=['train.csv'],
                            mode='val',
    ...
    # model = model_zoo.get_model(backbone=args.model)
    ...
    # criterion = BiTemperedLogisticLoss(t1=0.5, t2=1.5, smoothing=0.2)
    ...
    criterion = TaylorCrossEntropyLoss(n=6, ignore_index=255, reduction='mean',
                                       num_cls=NUM_CLASS, smoothing=0.2)
    ...
    def train(epoch):
        ...
        for batch_idx, (images, targets, fnames, indexs) in enumerate(tbar):
            ...
            if args.beta > 0 and r < args.cutmix_prob:
                ...
                # https://github.com/shengliu66/ELR
                # loss = criterion(indexs.cpu().detach().numpy().tolist(), outputs, target_a) * lam + \
                #        criterion(indexs.cpu().detach().numpy().tolist(), outputs, target_b) * (1. - lam)
            else:
                r = np.random.rand(1)
                if r < args.mixup_prob:
                    ...
                    # images2 = crop_images(images, 288)
                    outputs = model(images)
                    ...
                    # https://github.com/shengliu66/ELR
                    # loss = criterion(indexs.cpu().detach().numpy().tolist(), outputs, targets)
